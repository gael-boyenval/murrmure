...
| Meeting will not close / persistent Cursor remains | Update the Hub. **Close** / **Stop meeting** / header **Stop** closes the room and SIGTERMs every seat PTY immediately (then SIGKILL). `Dismiss` on a single run is not enough. |
