...
Build a brand-new **`daily-brief`** capability with `@studio/capability-sdk`.

Your canvas UI has one action: **Run daily brief**. Clicking it emits `brief.requested`, a trigger dispatches `mcp_wake`, your agent gathers from local tools, then submits formatted markdown/json output back to Studio for review.

## What you build

| Piece | Role |
|-------|------|
| **`daily-brief` capability** | Contract states + canvas button + `submit_brief_output` MCP tool |
...
## End-to-end Studio communication

1. Human clicks **Run daily brief** in the browser canvas.
2. Capability server appends `brief.requested` with payload (instance id, request id).
...
7. Capability instance moves `pending_agent` → `pending_review`.
...
2. [Push capability and register trigger](./02-push-and-trigger)
...
- Node.js 20+
- Studio hub access (cloud or self-hosted)
- Familiarity with [Capabilities tutorial](../../capabilities-tutorial)
