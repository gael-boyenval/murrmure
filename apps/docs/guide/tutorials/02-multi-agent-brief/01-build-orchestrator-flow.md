...
Create the smallest custom flow that supports:
...
## 1. Scaffold a clean flow project

```bash
mkdir -p ~/work/orchestrator && cd ~/work/orchestrator
npm init -y
npm install -D @murrmure/cli
mrmr flow init team-brief --dir ./flows/team-brief
cd flows/team-brief
```
...
| `published` | No pending work in Murrmure | Event `brief.published` emitted |
...
In `flow.manifest.json`, set `mcp_tools_by_version["1.0.0"]` to:
...
Copy `ui/src/lib/hub-client.ts` from [Tutorial 1 — React split UI scaffold](../01-local-preview-review/01-scaffold-flow) and keep it as your shared authenticated canvas client.
...
Use the same `server/hub.ts` helper as Tutorial 1 (`hubJson` with `MURRMURE_HUB_URL` + `MURRMURE_TOKEN`).
...
export function mountRoutes(app: { post: Function; get: Function; patch: Function }, ctx: FlowServerContext) {
...
From `~/work/orchestrator/flows/team-brief`:

```bash
mrmr flow validate . --json
mrmr flow build .
mrmr flow push --space spc_orchestrator --json
```
