...
Create a new custom flow from scratch using `@murrmure/cli`.
...
- Four MCP tools exposed by the flow
...
mrmr flow init preview-review --dir ./workflows/preview-review --install
cd ./workflows/preview-review
```

Why this step:

- `mrmr flow init` now scaffolds a strict React flow project
...
Then map them in `flow.manifest.json`:
...
```typescript
import type { HubBridgeClient } from "@murrmure/flow-dev-kit/react";
...
```tsx
import { useFlowContextPublic, useHubBridgeClient } from "@murrmure/flow-dev-kit/react";
