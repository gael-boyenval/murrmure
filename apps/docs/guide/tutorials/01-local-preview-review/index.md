...
- **Mixed orchestration** — shell prompts + nested build/review loop under **`build`**
- **`murrmure_resolve_step`** — agent completes **`build.build-loop`**; engine opens **`build.review`**
...
| 3 | Agent codes + review loop | **build** (parent) | prompt + `skills/feature-build/SKILL.md` |
| 3a | Agent reports preview URL | **build.build-loop** | resolve_step |
...
| **Agent-owned (nested)** | One **build** invoke; agent resolves **build.build-loop**; engine opens **build.review** | **Yes** |

The flow never re-invokes **`feature_build`** on feedback. `changes_required` uses **`continue: parent` + `goto: build-loop`**; the agent already running fixes locally.
