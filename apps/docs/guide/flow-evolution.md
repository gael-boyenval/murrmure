# Flow evolution pipeline

When you install a user-authored flow (for example **`app-live-review`**, **`preview-review`**, or any FDK package), the install moves through an **evolution pipeline** before agents and the runtime canvas can use it.

Open the install in **Configure → [space] → Flows → [install]** — you see the state strip, action buttons, and (when relevant) a contract diff panel.
...
On your machine, from the flow project:

```bash
export MURRMURE_HUB_URL=http://127.0.0.1:8787
export MURRMURE_TOKEN=tok_your_admin_grant
...
Record **`install_id`** from the push output or `mrmr flow status . --json`.

**Browser:** **Configure → Flows** — the row shows `app-live-review v… — draft`.
...
**When it fails:** Fix errors locally (`mrmr flow validate .`), rebuild, push again (same semver updates the draft install).

---

### Step 2 — Test → `tested`

**What it does:** Runs the flow's contract test entry (Vitest reachability / integration declared in the manifest).
...
**Promote updates evolution state only.** It does **not** spawn the flow worker or register MCP tools. You still need **Apply live** (Step 4) for FDK bundles.
:::
...
curl -s -H "Authorization: Bearer $MURRMURE_TOKEN" \
  "$MURRMURE_HUB_URL/v1/spaces/spc_ui_sandbox/flows/live" | jq .
```
...
```bash
curl -s -H "Authorization: Bearer $MURRMURE_TOKEN" \
  "$MURRMURE_HUB_URL/api/app-live-review/health"
```
...
| Goal | Browser | CLI |
|------|---------|-----|
| Upload bundle | — | `mrmr flow push --space …` |
| Validate | **Validate** | `mrmr flow validate --space … --install …` |
| Test | **Test** | `mrmr flow test --space … --install …` |
| Promote | **Promote** | `mrmr flow promote --space … --install …` |
| Mount worker + MCP | — | `mrmr flow apply --space … --install …` |
...
|---------|--------------|-----|
| State says `live` but MCP tools missing | Promoted without **Apply live** | Run `mrmr flow apply …` |
| Stuck on `promoted_pending` | Gate not approved | **Runtime → Gates** on production space |
| `BUNDLE_DIGEST_MISMATCH` on push | Stale stage / digest sidecar | `mrmr flow build .` then push again |
...
- [Browser app — Flows](./browser#configure--flows)
- [Flows tutorial](./flows-tutorial) — Part 7–8 push and evolution
- [Agent skill](./agent-skill) — teaches coding agents this pipeline
- [Flow Dev Kit reference](../reference/flow-dev-kit)
