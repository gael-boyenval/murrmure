...
| `murrmure_start_meeting` | `flow:run` | `POST /v1/meetings` — convene (`participants`, `chair` required; `title`, `goal`, `session_id` optional). Closed `session_id` aliases `murrmure_resume_meeting` and ignores a new title / goal / roster. |
...
| `murrmure_resume_meeting` | `flow:run` | `POST /v1/sessions/{id}/meeting/resume` — `{ session_id }`. Same `ptc_*`, `trigger: resumed`, journals `mrmr.meeting.resumed`. |
