...
Murrmure artifacts move large payloads out of journal `data` and invoke bodies. The hub stores canonical bytes; spaces receive materialized copies under `.mrmr/dev/`.
...
| Artifact | TTL-bound (default 90 days) | `~/.murrmure/exchanges/{transfer_id}/` + `{space}/.mrmr/dev/` |
...
`?preview=1` adds `preview: { text, truncated, name } | null`. Text is capped (~32 KiB) and only for text-like names (`.md`, `.txt`, `.json`, `.diff`, …). Binary or unknown types return `null`. This is operator Transcript preview — not materialize, not `/bytes`, not a PR/diff renderer.

Scopes: `blob:read` or `space:read`. ACL still wins. Meeting `said` also adds `actor:{chair}` to `authorized_readers`.

### `GET /v1/sessions/{session_id}/artifacts/{transfer_id}?preview=1`

Same auth as the meeting transcript. The `xfr_*` must appear on a `said` in that room. Returns the same metadata/preview payload without requiring the caller to claim the sender space. `ARTIFACT_NOT_IN_MEETING` when the id is not on the transcript.
...
Copies exchange bytes to `{space_root}/.mrmr/dev/inbox/{transfer_id}/{name}` after digest verification.

### MCP `murrmure_put_artifact`

```json
{ "content": "inline bytes", "name": "note.txt" }
```

or `{ "path": "relative/to/space/root.txt" }`. Exactly one of `path` or
`content`. `name` is required with `content` and defaults from the path
basename. Inline `content` is capped at 64 KiB. `authorized_readers` is
optional and defaults to the caller space; meeting `said` expands the roster
when the `xfr_*` is referenced.

Scope: `blob:write` (catalog also accepts `space:write`).

Response: `{ "artifact": { "transfer_id", "digest", "name", "size_bytes" } }`.

### MCP `murrmure_get_artifact`

```json
{ "transfer_id": "xfr_01J…" }
```

Materializes the same verified copy into the authenticated caller space and
returns safe artifact metadata with relative `local_path`; it omits
`authorized_readers` from the tool response. The tool requires
`space:read`, but the artifact ACL remains authoritative: the caller space or
actor must be listed in `authorized_readers`. Regular tokens cannot select a
different target space. `artifact_id` is accepted as an MCP-edge input alias
for agent ergonomics; manifests and protocol events remain canonical
`transfer_id`.
...
| Path | Role |
|------|------|
| `{space}/.mrmr/dev/inbox/` | Received artifacts (gitignored) |
| `{space}/.mrmr/dev/outbox/` | Pending send staging |
| `~/.murrmure/exchanges/{transfer_id}/` | Canonical hub copy + recovery |

`mrmr space init` scaffolds `.mrmr/dev/` with a gitignore hint.
...
- Default TTL 90 days (override via `ttl_days` on put)
