...
**Binds:** [pitfalls.md](./pitfalls.md) D1, D4, D5, D7, D13 · [persistence.md](./persistence.md)
...
CLI: [cli.md](./cli.md). Shell: [shell-lens.md](./shell-lens.md).

---

## 2. Emit (the broken wire)

Today:

- MCP `murrmure_emit_event` → `POST /v1/spaces/:id/events` with `{ event_type, event_id, payload }`. **No `session_id`.**
- HTTP POST without `instance_id`: **no journal**, fake `{ seq: 1 }`, no `event:emit` check.
- Catalog / `events.yaml` do not actually gate emit.

**Required:**

1. Journal-first `emitAndDeliver` ([architecture.md](./architecture.md) R2).
2. Meeting types: `session_id` on the envelope (top-level or payload — **lock: top-level `session_id`** so attach cannot be dropped).
3. HTTP emit requires `event:emit` (parity with MCP).
4. Hub-only denylist: `mrmr.meeting.convened`, `delivered`, `delivery_failed` (and any other hub-authored type).
...
See [spec.md](./spec.md) §9 for the DTO. Wire rules:

- `since_seq` / `up_to_seq` = **meeting_seq** ([persistence.md](./persistence.md) §4.2)
