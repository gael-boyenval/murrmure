...
| fix | fix | glm-5.2-max | fixed | Closed all four re-review blockers within Task 06 scope. (1) Assignment scope: a shared `requireAssignmentScope` helper enforces run/step/space on **every `step:resolve` endpoint** — upload-intent creation (`POST /v1/runs/:run_id/steps/:step_id/upload-intents`), file transfer (`PUT /v1/upload-intents/:intent_id/files/:index`), intent abandon (`DELETE /v1/upload-intents/:intent_id`), and step resolve — checking `scope.space_id` → `harness_id` (`run:{run_id}`) → `scope_ref`. `mintRunResolveToken` now mints `scope_ref` `{run_id}:{step_id}:{handler_id}` (handler identity carried for audit/binding; a step binds one handler so run:step implies it) and `harness_id` `run:{run_id}`, so an ephemeral handler token can no longer create upload intents or transfer files for another active run/step (`TOKEN_RUN_SCOPE_MISMATCH` / `TOKEN_STEP_SCOPE_MISMATCH`) or another space (`scope_enforcement_failure`); non-ephemeral grant tokens keep the space-only boundary. `UploadIntentService` exposes `getIntentScope` so file-transfer/abandon routes re-derive the run/step/space from the intent. (2) Shutdown escalation: `killChildProcess` is idempotent (WeakMap) and returns an awaitable `Promise<void>` whose SIGKILL timer is **ref'd** (not `unref`'d); `cancelHandlesForKeys` collects each handle's `awaitTermination()` into `pendingTerminations`, and `awaitAllShellExecutorsTerminated()` is awaited in `main.ts` `shutdown()` before `kernelPersistence.close()`/`db.close()`/exit, so a TERM-resistant descendant can no longer outlive the daemon. (3) Destination containment: `materializeConsumerCopy` realpath-canonicalizes the consumer `inputs/{slot}` dir after `mkdir` against the same `realRunRoot` and rejects escape (`ARTIFACT_PATH_TRAVERSAL`), and `lstat`-rejects a pre-existing symlink at the destination filename — so a malicious `.../inputs/{slot}` symlink can no longer redirect the temp write and atomic rename outside the run scratch tree. (4) Once-only: `ShellSpawnDeps.onProcessStart` now returns the cancel-handle unregister from `registerShellProcessCancel`; the executor calls it in `finish()` (timeout/error/close for both detached and non-detached), so a timeout that locally terminates followed by the run-failure cancellation path (`cancelRunExecutors`) finds no handle and signals the group exactly once. | Focused root Vitest green: `assignment-scope` 10/10; new HTTP `step-work-upload-scope` 4/4 (cross-run `TOKEN_RUN_SCOPE_MISMATCH`, cross-step `TOKEN_STEP_SCOPE_MISMATCH`, cross-space `scope_enforcement_failure`, same-run passes the boundary); `run-executor-cancel` 6/6 (incl. "shutdown awaits the SIGKILL escalation before resolving" + idempotent re-cancel); `consumer-copy` 13/13 (incl. destination-parent symlink escape + pre-existing destination-filename symlink); `auth-credential-lifecycle` 4/4; `run-resolve-credential-registry` 6/6; `shell-command` 29/29; `shell-spawn` 10/10; `shell-spawn-safety` 9/9 (incl. integrated "timeout then run-failure cancel terminates the tree once with one result": one SIGTERM, one SIGKILL, `cancelRunExecutors` no-op); `invoke-service` 2/2; `upload-intent-service` 4/4; HTTP runs dir 12 passed/1 skipped; `tutorial-v3-http` 4 passed/2 skipped; artifacts 9 passed/2 skipped. Combined executors-conformance + hub-core invoke dirs: 91 passed/3 skipped. `pnpm typecheck` clean for hub-core, hub-daemon, executors. `pnpm check:boundaries` clean (542 modules, 1429 dependencies). Docs synced: ADR-012 amended (Decisions 3/6/7 + Enforcement), `current/bridges/handlers.md` updated (consumer copy, process lifecycle, Q6), root CHANGELOG `Fixed (second post-review, 2026-07-15)` added, `apps/docs` space-handlers/environment/troubleshooting updated. | review |
| re-review | review | gpt-5.6-sol-high | blocked | Re-reviewed clean fix-2 commit `68148cd`. Assignment scope, destination containment, and integrated timeout/run-failure once-only deregistration are covered, but the shutdown process-tree done gate still fails. `killChildProcess` registers `finish` on the shell leader's `close`/`exit`; `finish` clears the five-second escalation timer and resolves the promise. If the leader exits on `SIGTERM` while a TERM-resistant descendant remains in the process group, `awaitAllShellExecutorsTerminated()` returns and daemon shutdown may exit without sending group `SIGKILL`. This contradicts the contract that shutdown terminates the complete process tree and the code's own escalation comment. | All 14 focused files passed: executors 55/55 (`shell-command` 29, `shell-spawn-safety` 9, `shell-spawn` 10, `suite` 7); hub-core 28 passed/2 expected future-task skips (`consumer-copy` 13, credential registry 6, executor cancel 6, tutorial handler 3/2); hub-daemon 28/28 (`assignment-scope` 10, upload-scope HTTP 4, resolve-step HTTP 4, auth lifecycle 4, invoke service 2, upload-intent service 4). Total: 111 passed/2 skipped. A direct shutdown probe emitted leader `close` immediately after cancellation and observed `awaitResolved: true`, `escalationCleared: true`, and only `SIGTERM` signals—no `SIGKILL`. The new shutdown test keeps the leader open, so it misses this descendant-survival case. | Keep the process-group SIGKILL escalation armed and awaited even when the shell leader exits after SIGTERM; add a regression with leader exit plus a surviving TERM-resistant descendant, then re-review. |
| fix | fix | glm-5.2-max | fixed | Closed the shutdown process-tree blocker within Task 06 scope. `killChildProcess` no longer lets the shell leader's `close`/`exit` clear the SIGKILL escalation unconditionally: the leader-exit handler now resolves early only once a signal-0 probe (`process.kill(-pid, 0)`) confirms the **entire process group** is dead. A surviving TERM-resistant descendant keeps the group alive, so the ref'd five-second SIGKILL escalation stays armed, fires `SIGKILL` on the group after the grace period, and only then resolves the awaitable `Promise<void>` that `awaitAllShellExecutorsTerminated` collects at shutdown — so daemon shutdown waits for full group death instead of returning when the leader alone exits. Idempotency (WeakMap) and the ref'd timer are preserved; the once-only deregistration path is untouched. | Focused root Vitest green: `run-executor-cancel` 7/7 (added "SIGKILL escalation survives leader exit when a TERM-resistant descendant lives": leader `close` after SIGTERM, group alive, escalation armed through 4999ms, `SIGKILL` on `-6666` after grace, shutdown resolves; updated once-only test to fake timers + grace advance — one `SIGTERM`, one `SIGKILL`, no re-signal); executors conformance 59 passed/3 skipped (`shell-command` 29, `shell-spawn` 10, `shell-spawn-safety` 9, `suite` 7); hub-core 29 passed/2 skipped (`consumer-copy` 13, `run-resolve-credential-registry` 6, `run-executor-cancel` 7, `tutorial-v3-handler` 5/2); hub-daemon 28/28 (`assignment-scope` 10, `step-work-upload-scope` 4, `resolve-step` 4, `auth-credential-lifecycle` 4, `invoke-service` 2, `upload-intent-service` 4); desktop `lifecycle` 8/8. Total affected: 131 passed/5 skipped. `pnpm typecheck` clean for hub-core and hub-daemon. `pnpm check:boundaries` clean (542 modules, 1429 dependencies). Docs synced: ADR-012 amended (Decision 6 close-handler probe wording + Enforcement regression note), root CHANGELOG `Fixed (third post-review, 2026-07-15)` added; `current/bridges/handlers.md` and `apps/docs` space-handlers/troubleshooting already describe the corrected behavior. | review |
