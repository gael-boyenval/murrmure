...
| review | review | glm-5.2-max | approved | Reviewed Task 09 at latest commit `d6ae0d4` (working tree clean). All six done-gate bullets satisfied. (1) Tutorial admits at most one non-terminal run and queues nothing: `RunPolicySchema` (`packages/contracts/src/entities/handler.ts`, `max_concurrent_runs` int ≥ 1) + tutorial `handlers.yaml` `run_policies: [{ flow: my-dev-flow, max_concurrent_runs: 1 }]`; `admitFlowRun` (`run/admission.ts`) returns `FLOW_CONCURRENCY_LIMIT` with canonical flow identity, configured limit, and active blocker run IDs on overflow — no run/queue created. (2) Unbounded flows stay concurrent: no policy → `admitFlowRun` returns `{ ok: true }` immediately (HTTP `run-capacity` confirms concurrent admission). (3) Every start path enforces identical policy and errors: manual `flow-starts`, manual `sessions/:id/runs`, scheduled (cron), event (`matchFlowEventStarts`), hook/handler `dispatch.ts`, and orchestration `attach.ts` all route through `admitAndCreateRun`/`startFlowRun` → `admitFlowRun`; all surface `FLOW_CONCURRENCY_LIMIT` 409; trigger denial is journaled as `FLOW_START_DENIED` and retry performs a fresh admission check. (4) Apply cannot replace configuration during any non-terminal run: `assertSpaceQuiescent` → `SPACE_HAS_ACTIVE_RUNS` 409 with blocker IDs, executed inside `ctx.spaceRunGuard.with(...)` held from the quiescence check through `replaceSpaceIndex` commit (`routes/spaces/index.ts`). (5) Runs and journals pin the configuration they actually used: `startFlowRun` and `admitAndCreateRun` re-fetch the canonical `getFlowIndexEntry` inside the guard and pin `flow_digest = entry.digest` (caller-supplied digests ignored, `RUN_STARTED` records the pinned digest); apply quiescence guarantees no apply can commit between admission and dispatch, so post-admission dispatch reads the same IR. (6) Portable flows contain no concurrency policy: `my-dev-flow/flow.manifest.yaml` carries only triggers/steps/branches; capacity is space-owned in `handlers.yaml`. Shared guard verified: `SpaceConcurrencyGuard` (`run/space-guard.ts`) is one `ctx.spaceRunGuard` singleton shared by apply and all run deps (`flowRunDeps`, `sessionRunDeps`); `admitFlowRun`/`assertSpaceQuiescent` dropped their `limit` caps so every active blocker ID is returned. ADR-011 present. **Notes (non-blocking):** (a) Co-commit history — the bulk of Task 09 was co-committed in `495435e` (task-05 build), already disclosed in the build handoff and the task-05 review; the task-09 build commit `d6ae0d4` additionally co-mingles Task 06/11 scope — `packages/executors/src/shell-command.ts` (+25-test conformance suite), `shell-spawn.ts` behavior switch to `resolveSafeShellCommand`, `flow-engine/run-scratch-paths.ts`, `flow-engine/consumer-copy.ts`, the `step-artifacts`/`step-contract-slice` refactor, and the `executors/package.json` `@murrmure/hub-core` dep (ADR-012 / run-scratch-path-normalize and the Part 5 shell grammar). These do not touch the Task 09 done gate or its 46 tests, but `shell-spawn.ts` is Task 06's hot file ("shell execution and placeholder expansion | 06 only") and its behavior change must be validated under Task 06; per rule #2 tasks should land in separate commits — recommend attributing/reviewing the co-mingled Task 06/11 code under its own task and splitting `d6ae0d4` at Task 13 cutover. (b) Build handoff's "Digest-preparation unit regressions: 10" / "Guard-adapter regressions: 4" labels do not map to distinct files — the digest-pinning and guard-adapter coverage is already inside the 6 focused suites counted above (the only other new suite, `shell-command.test.ts`, is Task 06 with 25 tests). | Task 09 focused suites: 6 files, 46/46 passed (`run-policy` 16, `admission` 10, `run-capacity-races` 6, `space-guard` 5, HTTP `run-capacity` 5, HTTP `apply-quiescence` 4). Typecheck: `@murrmure/hub-core`, `@murrmure/hub-daemon`, `@murrmure/executors` clean. Lints: clean on the 7 edited Task 09 files. ADR-011 present; tutorial `handlers.yaml` carries `max_concurrent_runs: 1`; portable flow manifest carries no concurrency policy. | Task 09 approved. Non-blocking: attribute/review the co-mingled Task 06/11 files (`shell-command`, `run-scratch-paths`, `consumer-copy`, `shell-spawn`) under their own tasks and validate the `shell-spawn` grammar change under Task 06; consider split-to-prs for `d6ae0d4` at Task 13 cutover. |
