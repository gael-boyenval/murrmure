...
export const TUTORIAL_MEETINGS_FIXTURE_ROOT = resolve(
  import.meta.dirname,
  "../spaces/tutorial-meetings",
);

export type TutorialMeetingsPart = 2 | 3 | 5 | 6;
...
export interface TutorialMeetingsSnapshot {
  part: TutorialMeetingsPart;
  source: string;
...
function loadSnapshotFile(path: string, seen: Set<string>): TutorialMeetingsSnapshot {
...
export function tutorialMeetingsSnapshotPath(part: TutorialMeetingsPart): string {
  return join(TUTORIAL_MEETINGS_FIXTURE_ROOT, `part-${part}`, "snapshot.json");
}

export function loadTutorialMeetingsSnapshot(
  part: TutorialMeetingsPart,
): TutorialMeetingsSnapshot {
  return loadSnapshotFile(tutorialMeetingsSnapshotPath(part), new Set());
}

export function materializeTutorialMeetingsSnapshot(
  part: TutorialMeetingsPart,
  destination: string,
): TutorialMeetingsSnapshot {
...
export function resetMaterializedTutorialMeetingsSnapshot(
  part: TutorialMeetingsPart,
  destination: string,
): TutorialMeetingsSnapshot {
  rmSync(destination, { recursive: true, force: true });
  return materializeTutorialMeetingsSnapshot(part, destination);
}
