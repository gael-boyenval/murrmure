...
const PLATFORM_TOOLS: Array<{
  ...
  { name: "murrmure_list_memory_banks", required_scope: "memory:read", description: "List Memory banks this caller may read (own bank plus granted foreign banks)" },
  ...
  { name: "murrmure_start_meeting", required_scope: "flow:run", description: "Convene a meeting, or resume a closed meeting when session_id is already a room" },
  ...
  { name: "murrmure_resume_meeting", required_scope: "flow:run", description: "Reopen a closed meeting on the same session (same ptc_* and Cursor chats)" },
