...
import { DevSimCapabilityMount, type CapabilityMountResponse } from "./capability-mount.js";
import {
  DEV_SIM_FIXTURES,
  parseExpectedRevision,
  parseFixture,
  parseInstanceActor,
  SimulatedStudioMachine,
  type DevSimFixture,
...
interface ServerState {
  ...
  stageDir: string;
  manifest: DevSimManifest;
  machine: SimulatedStudioMachine;
  bundleDigest: string;
...
<html lang="en">
  ...
  <body>
    <header>
      <strong>studio capability dev --sim</strong>
...
export async function startDevSimServer(opts: DevSimServerOptions): Promise<DevSimServerHandle> {
  ...
  let state: ServerState = {
    ...
    machine: new SimulatedStudioMachine(opts.contract, opts.fixture ?? "live-install-ready"),
    bundleDigest: opts.bundleDigest,
    activeFixture: opts.fixture ?? "live-install-ready",
  };

  const capabilityMount = new DevSimCapabilityMount({
  ...
  return {
    ...
    reload: (next) => {
      ...
      state = {
        ...
        machine: new SimulatedStudioMachine(next.contract, fixture),
