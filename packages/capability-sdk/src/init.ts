...
import { examplesRoot } from "./paths.js";
import { installStudioSkill } from "@studio/skill";

const SCAFFOLD_DEPENDENCY_VERSIONS = {
  "@studio/capability-dev-kit": "0.1.0",
  react: "18.3.1",
  "react-dom": "18.3.1",
};
...
const ROOT_PACKAGE_JSON = (id: string) => ({
  ...
  type: "module",
  scripts: {
    "validate:capability": "studio capability validate .",
    "build:capability": "studio capability build .",
    "dev:capability": "studio capability dev . --sim",
...
const UI_APP = `import { useCapabilityContextPublic, useHubBridgeClient } from "@studio/capability-dev-kit/react";
import { useCallback, useEffect, useState } from "react";
import { CapabilityErrorState } from "./components/error/CapabilityErrorState";
  ...
  return (
    <main style={{ fontFamily: "system-ui, sans-serif", padding: "1rem" }}>
      <header style={{ marginBottom: "1rem" }}>
        <h1 style={{ marginBottom: "0.5rem" }}>Capability canvas</h1>
        <p style={{ margin: 0 }}>
          <strong>{ctx.packageId}</strong> in <strong>{ctx.spaceId}</strong> / instance{" "}
...
const UI_MOUNT = `import type { CapabilityHostContext } from "@studio/capability-sdk/host";
...
const CAPABILITY_ERROR_STATE = `import {
  CapabilityErrorState as DevKitCapabilityErrorState,
  type CapabilityErrorStateProps,
} from "@studio/capability-dev-kit/react";

export function CapabilityErrorState(props: CapabilityErrorStateProps) {
  return (
    <DevKitCapabilityErrorState
      title={props.title ?? "Capability runtime error"}
...
const CAPABILITY_ERROR_BOUNDARY = `import { CapabilityErrorBoundary as DevKitCapabilityErrorBoundary } from "@studio/capability-dev-kit/react";
import type { ReactNode } from "react";
import { CapabilityErrorState } from "./CapabilityErrorState";

export function CapabilityErrorBoundary({ children }: { children: ReactNode }) {
  return (
    <DevKitCapabilityErrorBoundary
      fallback={({ error, reset }) => (
        <CapabilityErrorState
          message={error.message}
          details={error.stack}
          onRetry={reset}
          retryLabel="Retry capability"
        />
      )}
    >
      {children}
...
const port = Number(process.env.CAPABILITY_SIM_PORT ?? 4310);
...
export default defineConfig({
  ...
  webServer: {
    command: \`npm run dev:capability -- --port \${port}\`,
    port,
...
export function capabilityCanvasFrame(page: Page): FrameLocator {
  return page.frameLocator("#capability-canvas");
}
`;
...
import { capabilityCanvasFrame } from "./harness/simulated-shell";
import { applyFixture } from "./harness/simulated-studio-machine";

test("renders capability canvas from simulator", async ({ page, request }) => {
  await applyFixture(request, "live-install-ready");
  await page.goto("/");
  const frame = capabilityCanvasFrame(page);
  await expect(frame.getByRole("heading", { name: "Capability canvas" })).toBeVisible();
...
export interface InitCapabilityOptions {
  install?: boolean;
  packageManager?: "npm";
  fromExample?: string;
  withSkill?: boolean;
...
const EXAMPLE_SKIP = new Set(["node_modules", "dist", ".studio", ".git"]);
...
function initFromExample(
  ...
  dir: string,
  example: string,
  opts?: InitCapabilityOptions,
): { ok: boolean; path: string; installed: boolean } {
  const source = join(examplesRoot(), example);
  if (!existsSync(join(source, "capability.manifest.json"))) {
  ...
  if (opts?.withSkill) {
    installStudioSkill(process.cwd());
  }
...
export function initCapability(
  ...
  writeFileSync(join(dir, "capability.manifest.json"), JSON.stringify(DEFAULT_MANIFEST(id), null, 2));
  ...
  writeFileSync(
    join(dir, "ui", "src", "components", "error", "CapabilityErrorState.tsx"),
    CAPABILITY_ERROR_STATE,
  );
  writeFileSync(
    join(dir, "ui", "src", "components", "error", "CapabilityErrorBoundary.tsx"),
  ...
  writeFileSync(
    join(dir, "tests", "e2e", "harness", "simulated-studio-machine.ts"),
