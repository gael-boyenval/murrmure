...
export function validateCapabilityRoot(dir: string, opts?: { postBuild?: boolean }): ValidateResult {
  ...
  try {
    ...
    return {
      ok: false,
      errors: [{ code: "MANIFEST_INVALID", message: "capability.manifest.json is not valid JSON" }],
      warnings,
    };
  }

  const legacy = LegacyCapabilityManifestSchema.safeParse(raw);
  if (legacy.success) {
    warnings.push({
  ...
  if (!opts?.postBuild) {
    ...
    if (!existsSync(packageJsonPath)) {
      errors.push({
        ...
        hint: { file: packageJsonPath, dependency: "@studio/capability-dev-kit" },
      });
    } else {
      ...
      const devKitVersion = readDependencyVersion(packageJson, "@studio/capability-dev-kit");
      if (!devKitVersion) {
        errors.push({
          code: "DEVKIT_VERSION_REQUIRED",
          message: "Missing @studio/capability-dev-kit dependency",
          hint: { file: "package.json", dependency: "@studio/capability-dev-kit" },
        });
      }
      ...
      const sdkVersion = readDependencyVersion(packageJson, "@studio/capability-sdk");
      if (sdkVersion && devKitVersion && sdkVersion !== devKitVersion) {
        errors.push({
          code: "DEVKIT_SDK_VERSION_MISMATCH",
          message:
            "@studio/capability-sdk and @studio/capability-dev-kit must use matching versions",
          hint: {
            file: "package.json",
