...
export async function pushCapability(opts: PushOptions): Promise<Record<string, unknown>> {
  ...
  const manifest = JSON.parse(readFileSync(join(sourceDir, "capability.manifest.json"), "utf-8")) as {
    id: string;
    version: string;
  };
  const stageDir = stagePath(manifest.id, manifest.version);
  const digestFromFile = readFileSync(join(stageDir, "bundle.digest"), "utf-8").trim();
  const digest = await computeBundleDigest(stageDir);
  if (digest !== digestFromFile) {
    return {
      ok: false,
      code: "BUNDLE_DIGEST_STALE",
      message:
        "Staged bundle.digest does not match recomputed digest — re-run `studio capability build` before push",
  ...
  const res = await hubFetch(auth, `/v1/spaces/${opts.spaceId}/capabilities/install`, {
    method: "POST",
    json: {
      ...
      source_metadata: {
        ...
        sdk_version: buildMeta.sdk_version ?? "0.1.0",
      },
      config: {},
      target_state: "draft",
    },
...
export async function evolutionCommand(
  ...
  const paths: Record<string, { method: string; path: string }> = {
    ...
    apply: { method: "POST", path: `/v1/spaces/${opts.spaceId}/capabilities/${opts.installId}/apply` },
...
export function readPushState(packageId: string, version: string): PushState | null {
  try {
    return JSON.parse(readFileSync(pushStatePath(packageId, version), "utf-8")) as PushState;
  } catch {
    return null;
  }
}
...
export async function doctor(): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    if (!whoami.ok) issues.push({ code: "TOKEN_DENIED", message: "Token rejected" });
      ...
      const hasInstall = body.spaces?.some((s) => s.scopes?.includes("capability:install"));
      if (!hasInstall) issues.push({ code: "SCOPE_MISSING", message: "Missing capability:install scope" });
    }
  } catch (e) {
