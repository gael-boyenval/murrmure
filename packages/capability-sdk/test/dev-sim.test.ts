...
import { devCapabilityLoop, initCapability } from "../src/index.js";
...
describe("dev --sim", () => {
  test("starts thin server with install + instance state machine", async () => {
    ...
    try {
      initCapability("demo-sim", dir);
      linkScaffoldWorkspaceDeps(dir);
      loop = await devCapabilityLoop({
        path: dir,
        sim: true,
        simPort: 0,
        debounceMs: 50,
      ...
      const healthBody = (await healthResponse.json()) as { ok?: boolean; package?: string };
      expect(healthBody.ok).toBe(true);
      expect(healthBody.package).toBe("demo-sim");
    } finally {
      loop?.stop();
