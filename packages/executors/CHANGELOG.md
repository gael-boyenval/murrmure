...
- Meeting handlers may define a harness-owned continuation command and stdout
  token field. The executor persists the opaque token per `ses_*` / `ptc_*`
  and resolves `{{continuation_token}}` on later one-shot processes.
