...
const templatesRoot = join(here, "..");
const repoRoot = join(templatesRoot, "..", "..");
...
const requireFromDaemon = createRequire(join(repoRoot, "packages", "murrmure-hub-daemon", "package.json"));
const { register } = await import(pathToFileURL(requireFromDaemon.resolve("tsx/esm/api")).href);
register();

const sdkEntry = join(repoRoot, "packages", "murrmure-cli", "src", "index.ts");
const { buildFlowRoot } = await import(pathToFileURL(sdkEntry).href);

const targets = readdirSync(templatesRoot).filter((name) => {
