...
description: >-
  Authoring skill for `.mrmr/` spaces: handlers, meeting seats, directives,
...
my-space/
  .mrmr/
    space/
      space.yaml              # slug, name, description (purpose), link.space_id
...
Deep dives: [reference/space-directory.md](reference/space-directory.md), [reference/flow-authoring.md](reference/flow-authoring.md), [reference/meeting-seat.md](reference/meeting-seat.md), [reference/directive.md](reference/directive.md).

## This space should join meetings

Do this **in this repo** before convene can spawn anyone here. Full recipe:
...
Then `mrmr space apply --strict` and grant `event:emit` on this space’s connection.

## This space should accept directives

Do this **in this repo** so header **New directive** can pick it. Full recipe:
[reference/directive.md](reference/directive.md).

Handler only — do not copy a flow. One-shot `cursor agent -p`, not a persistent
PTY. Resolve `completed` / `failed` with `{ message }`.
...
Then `mrmr space apply --strict`. Default handler timeout is 30s — omit `timeout_ms` and `cursor agent -p` is killed mid-work.
...
Meeting seats: [This space should join meetings](#this-space-should-join-meetings) and [reference/meeting-seat.md](reference/meeting-seat.md).
