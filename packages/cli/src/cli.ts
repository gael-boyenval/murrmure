...
import { runHubCommand } from "./hub-commands.js";
...
async function runFlowCommand(
  ...
  switch (command) {
        ...
        out(
          {
            ok: false,
            code: "MISSING_ARG",
            message: "usage: mrmr flow init <id> [--dir path] [--from-example <name>]",
    ...
    case "build": {
      out(await buildFlowRoot(pathArg), json);
      break;
    }
    case "push": {
      if (!flags.space) {
      ...
      out(await pushFlow({ spaceId: String(flags.space), path: pathArg }), json);
      break;
    }
    case "status": {
      const manifest = JSON.parse(readFileSync(resolve(pathArg, "flow.manifest.json"), "utf-8")) as {
        id: string;
        version: string;
      };
    ...
    case "list": {
      ...
      const res = await hubFetch(auth, `/v1/spaces/${flags.space}/flows`);
...
async function main() {
  ...
  if (scope === "skill") {
    const { runMurrmureSkillCli } = await import("./skill/cli.js");
    await runMurrmureSkillCli([command, ...rest].filter((a): a is string => Boolean(a)));
    return;
  }
  if (scope === "flow") {
    const { flags, positional } = parseArgs(rest);
    await runFlowCommand(command, flags, positional);
    return;
  }
  if (scope === "mcp") {
  ...
  if (!scope || scope === "--help" || scope === "-h") {
    console.log("usage: mrmr flow|skill|mcp|<hub-command> ...");
    process.exit(scope ? 0 : 1);
    return;
  }

  await runHubCommand(scope, rest);
}
