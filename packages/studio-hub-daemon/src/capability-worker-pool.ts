...
/**
 * Build a minimal environment for a capability worker. Capability code is
 * untrusted, so it must not inherit the hub's full process environment (which
 * may carry API keys, tokens, or cloud credentials). Only a small host
 * allowlist plus the explicit STUDIO_* identity vars are forwarded.
 */
export function sanitizedWorkerEnv(studio: Record<string, string>): NodeJS.ProcessEnv {
    ...
    const proc = spawn(
      ...
      ],
      {
        env: sanitizedWorkerEnv({
          STUDIO_SPACE_ID: opts.spaceId,
          STUDIO_INSTALL_ID: opts.installId,
          STUDIO_PACKAGE_ID: opts.packageId,
          STUDIO_VERSION: opts.version,
          STUDIO_CONTRACT_REF_ID: opts.contractRefId,
          STUDIO_WORKER_TOKEN: workerToken,
          STUDIO_BRIDGE_PORT: String(opts.bridgePort),
          STUDIO_INSTALL_CONFIG: JSON.stringify(opts.installConfig ?? {}),
        }),
