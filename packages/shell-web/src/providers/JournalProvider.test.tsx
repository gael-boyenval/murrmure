...
describe("JournalProvider", () => {
  ...
  it("invalidates run and graph queries on journal.append", async () => {
    ...
    await waitFor(() => {
      ...
      expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: ["session-transcript", "ses_abc"] });
      expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: ["space-home", "spc_demo"] });
    });
  });

  it("invalidates transcript on said without touching Needs-you", async () => {
    const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    const invalidateSpy = vi.spyOn(queryClient, "invalidateQueries");
    ...
    await waitFor(() => {
      expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: ["session-transcript", "ses_room"] });
    });
