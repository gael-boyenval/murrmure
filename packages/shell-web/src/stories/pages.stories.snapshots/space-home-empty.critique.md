...
Space home is the default landing for a linked space in observer mode. With no active work, it should orient the user toward CLI setup (`mrmr space apply`, flow init/push) while keeping the shell readable and calm. Per spec §12.3, six sections exist when populated; empty states should degrade gracefully without wizard UI.

This snapshot shows **Demo space** with four cards: Active runs, Your flows (with CLI apply hint), Available to run, and Recent completed — all in placeholder mode. Receiving from is hidden when empty, matching production.
...
- **CLI-first empty copy on Your flows.** Elevated empty state: explanation line plus monospace `mrmr space apply` in a bordered code block — on-brand; no fake Run buttons or install wizards.
- **Available to run empty placeholder.** `No runnable flows` section mirrors production route surface (CC-11).
...
- **No differentiated empty-state treatment.** Your flows (actionable via CLI) and Active/Recent (passive wait) use identical gray one-liners — missed hierarchy opportunity.
...
- **~~Missing spec sections even as empty placeholders.~~** Resolved (CC-11): Available to run shown; Receiving from hidden when empty per production.
...
- **~~Gap vs production route~~** — prototype now mirrors five section cards (four visible when receiving_from empty).
...
1. **~~Add missing section cards~~** — done (CC-11).
2. **~~Elevate Your flows empty state~~** — done: CLI code block + apply explanation.
...
4. **Add first-run story variant** — sidebar with zero spaces → `/spaces/new` instruction page (separate story, but empty home should reference it when `your_flows` empty).
...
**Headline:** Calm, CLI-honest empty home — CC-11 sections and Your flows CLI block land; consider collapsing passive Active/Recent empties next.
