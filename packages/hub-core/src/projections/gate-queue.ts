...
export function gateQueueHandler(
  ...
  if (entry.type === "mrmr.gate.pending") {
    const cp = entry.payload as { gate_id?: string; checkpoint_id?: string; transition_id?: string };
    const gateId = cp.gate_id ?? cp.checkpoint_id ?? "";
    if (gateId) {
      state.pending.push({
        gate_id: gateId.replace(/^gate_/, ""),
        instance_id: entry.aggregate_id ?? "",
        transition_id: cp.transition_id ?? "",
        status: "pending",
      });
  ...
  if (entry.type === "mrmr.gate.resolved") {
    const cp = entry.payload as { gate_id?: string; checkpoint_id?: string };
    const gateId = (cp.gate_id ?? cp.checkpoint_id ?? "").replace(/^gate_/, "");
    state.pending = state.pending.filter((g) => g.gate_id !== gateId);
  }
